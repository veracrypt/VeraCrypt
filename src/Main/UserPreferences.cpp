...
#define TC_CONFIG_SET(NAME) if (configMap.count(L###NAME) > 0) { SetValue (configMap[L###NAME], NAME); configMap.erase (L###NAME); }
			...
			if (configMap.count(L"MountExfatWithKernelDriver") > 0) { SetValue (configMap[L"MountExfatWithKernelDriver"], DefaultMountOptions.MountExfatWithKernelDriver); configMap.erase (L"MountExfatWithKernelDriver"); }
#endif
...
#define TC_CONFIG_ADD(NAME) formatter.AddEntry (L###NAME, NAME)
		...
		formatter.AddEntry (L"MountExfatWithKernelDriver", DefaultMountOptions.MountExfatWithKernelDriver);
#endif
